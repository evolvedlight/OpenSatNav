...
public class OpenStreetMapViewTraceOverlay extends OpenStreetMapViewOverlay implements Serializable {
	...
	protected List<RecordedGeoPoint> trace;
	protected ArrayList<RecordedWayPoint> wayPoints;
	protected RecordedGeoPoint mLocation;
	protected ArrayList<RecordedGeoPoint> pointRoute;
	...
	public OpenStreetMapViewTraceOverlay(final Context ctx, RouteRecorder mRouteRecorder) {
		...
		this.trace = mRouteRecorder.getRecordedGeoPoints();
		this.wayPoints = mRouteRecorder.getRecordedWayPoints();
		pointRoute = new ArrayList<RecordedGeoPoint>();
		pixelRoute = new ArrayList<Point>();
		routePath = new Path();
		for (int i = 0; i < this.trace.size(); i++) {
			RecordedGeoPoint nextPoint = this.trace.get(i);
	...
	public void setLocation(final RecordedGeoPoint mp) {
		this.mLocation = mp;
	}
	...
	public void onDraw(final Canvas c, final OpenStreetMapView osmv) {
		...
		if (this.pointRoute.size() != 0) {
			...
			for (RecordedWayPoint wayPoint : wayPoints) {
				Point wayPointLocation = pj.toPixels((GeoPoint) wayPoint, null);
				//13??? thats 1/2 the size of the star_on image, to get it in the right place!
				
				 //make it transarent so we can see user dot
				c.drawBitmap(BitmapFactory.decodeResource(osmv.getResources(), android.R.drawable.star_on), wayPointLocation.x - 13, wayPointLocation.y - 13, wayPointPaint);
			}
		}
